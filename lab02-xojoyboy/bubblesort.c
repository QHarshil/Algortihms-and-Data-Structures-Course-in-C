...
void swap(int* a, int* b) {
    // Swap integers with temp variable.
    int temp = *a;
    *a = *b;
    *b = temp;
}
...
void bubblesort(int* array, unsigned int size) {
    ...
    for (int i = 0; i < size - 1; i++) {
        // go through the start of the array to the end of the unsorted section
        // added -i to optimize and only iterate the unsorted section
        for (int j = 0; j < size - 1; j++) {
            // check if the current element is greater than the next element
            if (array[j] > array[j + 1]) {
                // if current element > next element, use swap function to swap their positions
