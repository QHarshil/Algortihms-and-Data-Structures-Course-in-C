...
void swap(int* a, int* b) {
    // Adding a temp variable to allow for switching address of two pointers
    int temp = *a;
    *a = *b;
    *b = temp;
}
...
// Since for insertion sort, the first element is already sorted
    for (unsigned int currentIndex = 1; currentIndex < size; currentIndex++) {
        // The current element to be placed into its correct sorted position
        int currentElement = array[currentIndex];

        // The last index of the sorted subsection of the array
        int sortedIndex = currentIndex - 1;

        // Move elements that are greater than the current element to one position ahead
        // of their current position by swapping
        while (sortedIndex >= 0 && array[sortedIndex] > currentElement) {
            // swap larger element with the position right next to it
            swap(&array[sortedIndex + 1], &array[sortedIndex]);
            sortedIndex = sortedIndex - 1;  //Move to left element in sorted section
        }        
    }
}
